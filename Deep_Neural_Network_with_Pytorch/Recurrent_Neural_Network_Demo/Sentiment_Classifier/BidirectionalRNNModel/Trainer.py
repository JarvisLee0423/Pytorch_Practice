...
from Model.BidirectionalRNNModel import BidirectionalRNNModelNN
...
def trainer(textField, trainSet, devSet):
    ...
    ''')
    # Creating the visdom.
    vis = Visdom(env = 'BidirectionalRNNModel')
    # Creating the graph.
    ...
    model = BidirectionalRNNModelNN(Cfg.vs + 2, Cfg.es, Cfg.hs, Cfg.cs, textField.vocab.stoi[textField.pad_token]).to(device)
    ...
    for epoch in range(Cfg.epoches):
        ...
        evalLoss, evalAcc = evaluator(model.eval(), loss, textField, devSet)
        ...
        # Saving the model.
        torch.save(model.train().state_dict(), Cfg.modelDir + f'/{currentTime}/BidirectionalRNNModel-Epoch{epoch + 1}.pt')
        # Converting the model state.
        model = model.train()
    # Saving the graph.
    vis.save(envs = ['BidirectionalRNNModel'])
    ...
    while cmd != 'Exit()':
        ...
        if cmd == 'T':
            ...
            try:
                ...
                model = BidirectionalRNNModelNN(Cfg.vs + 2, Cfg.es, Cfg.hs, Cfg.cs, textField.vocab.stoi[textField.pad_token]).to(device)
                # Loading the model.
                model.load_state_dict(torch.load(Cfg.modelDir + f'/{currentTime}/BidirectionalRNNModel-Epoch{Cfg.epoches}.pt'))
