...
from Model.CNNModel import CNNModelNN
...
def trainer(textField, trainSet, devSet):
    ...
    model = CNNModelNN(Cfg.vs + 2, Cfg.es, Cfg.cs, textField.vocab.stoi[textField.pad_token]).to(device)
    ...
    for epoch in range(Cfg.epoches):
        ...
        # Saving the model.
        torch.save(model.train().state_dict(), Cfg.modelDir + f'/{currentTime}/CNNModel-Epoch{epoch + 1}.pt')
        # Converting the model state.
        model = model.train()
    # Saving the graph.
    vis.save(envs = ['CNNModel'])
    ...
    while cmd != 'Exit()':
        ...
        if cmd == 'T':
            ...
            try:
                ...
                model = CNNModelNN(Cfg.vs + 2, Cfg.es, Cfg.cs, textField.vocab.stoi[textField.pad_token]).to(device)
                # Loading the model.
                model.load_state_dict(torch.load(Cfg.modelDir + f'/{currentTime}/CNNModel-Epoch{Cfg.epoches}.pt'))
