...
class AttentionGRUModelNN(nn.Module):
    ...
    def trainer(enTrainSet, cnTrainSet, enTrainLen, cnTrainLen, enDevSet, cnDevSet, enDevLen, cnDevLen, enVocabSize, cnVocabSize, hiddenSize, batchSize, learningRate, epoches):
        ...
        attention = Attention(hiddenSize).to(device)
        # Creating the decoder.
        decoder = Decoder(cnVocabSize, hiddenSize, attention).to(device)
        # Creating the sequence to sequence model.
        ...
        for epoch in range(epoches):
            ...
            evalLoss, evalAcc = AttentionGRUModelNN.evaluator(enDevSet, cnDevSet, enDevLen, cnDevLen, model.eval(), loss)
            ...
            if len(evalAcces) == 0 or evalAcc >= max(evalAcces):
                ...
                print("Model Saved")
                # Saving the model.
                torch.save(encoder.train().state_dict(), './Deep_Neural_Network_with_Pytorch/Recurrent_Neural_Network_Demo/Machine_Translation/AttentionEncoder.pt')
                torch.save(decoder.train().state_dict(), './Deep_Neural_Network_with_Pytorch/Recurrent_Neural_Network_Demo/Machine_Translation/AttentionDecoder.pt')
...
if __name__ == "__main__":
    ...
    #     if cmd == 'T':
    #         # Training the model.
    #         AttentionGRUModelNN.trainer(enTrainSet, cnTrainSet, enTrainLen, cnTrainLen, enDevSet, cnDevSet, enDevLen, cnDevLen, len(enItos), len(cnItos), hiddenSize, batchSize, learningRate, epoches)
    #         # Getting the command.
    #         cmd = input("Please input the command ('T' for training, 'E' for evaluating, 'Exit' for quit): ")
    #     elif cmd == 'E':
    ...
    #             attention = Attention(hiddenSize).to(device)
    #             decoder = Decoder(len(cnItos), hiddenSize, attention)
    #             # Loading the model.
    #             encoder.load_state_dict(torch.load('./Deep_Neural_Network_with_Pytorch/Recurrent_Neural_Network_Demo/Machine_Translation/AttentionEncoder.pt'))
    #             decoder.load_state_dict(torch.load('./Deep_Neural_Network_with_Pytorch/Recurrent_Neural_Network_Demo/Machine_Translation/AttentionDecoder.pt'))
