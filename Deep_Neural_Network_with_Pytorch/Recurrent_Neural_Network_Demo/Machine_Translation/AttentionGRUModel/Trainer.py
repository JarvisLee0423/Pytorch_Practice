...
from Model.AttentionGRUModel import Encoder, Decoder, Attention, AttentionGRUModelNN
...
def trainer(enTrainSet, cnTrainSet, enTrainLen, cnTrainLen, enDevSet, cnDevSet, enDevLen, cnDevLen, enVocabSize, cnVocabSize):
    ...
    # Logging the configurator's information.
    logging.info(f'''
    ...
    ''')
    # Creating the visdom.
    vis = Visdom(env = 'AttentionGRUModel')
    # Creating the graph.
    ...
    attention = Attention(Cfg.hs).to(device)
    # Creating the decoder.
    decoder = Decoder(cnVocabSize, Cfg.hs, attention).to(device)
    # Creating the sequence to sequence model.
    model = AttentionGRUModelNN(encoder, decoder).to(device)
    ...
    for epoch in range(Cfg.epoches):
        ...
        with tqdm(total = len(enTrainSet), desc = f'Epoch {epoch + 1}/{Cfg.epoches}', unit = 'batches', dynamic_ncols = True) as pbars:
            for i, bacth in enumerate(enTrainSet):
                ...
                pbars.update(1)
                # Updateing the training information.
        ...
        # Saving the model.
        try:
            torch.save(encoder.train().state_dict(), Cfg.modelDir + f'/Encoder-{currentTime}/AttentionEncoder-Epoch{epoch + 1}.pt')
            torch.save(decoder.train().state_dict(), Cfg.modelDir + f'/Decoder-{currentTime}/AttentionDecoder-Epoch{epoch + 1}.pt')
        except:
        ...
        model = model.train()
    # Storing the graph.
    vis.save(envs = ['AttentionGRUModel'])

# Setting the main function.
if __name__ == "__main__":
    # Getting the training data.
    enItos, enStoi, cnItos, cnStoi, enTrainSet, cnTrainSet, enTrainLen, cnTrainLen = trainComponentsGenerator.trainComponentsGenerator(Cfg.bs, Cfg.vs, Cfg.dataDir, shuffle = True, train = True)
    # Getting the development data.
    _, _, _, _, enDevSet, cnDevSet, enDevLen, cnDevLen = trainComponentsGenerator.trainComponentsGenerator(Cfg.bs, Cfg.vs, Cfg.dataDir, shuffle = False, train = False)
    ...
    while cmd != 'Exit':
        if cmd == 'T':
            ...
            try:
                ...
                attention = Attention(Cfg.hs).to(device)
                decoder = Decoder(len(cnItos), Cfg.hs, attention)
                # Loading the model.
                encoder.load_state_dict(torch.load(Cfg.modelDir + f'/Encoder-{currentTime}/AttentionEncoder-Epoch{Cfg.epoches}.pt'))
                decoder.load_state_dict(torch.load(Cfg.modelDir + f'/Decoder-{currentTime}/AttentionDecoder-Epoch{Cfg.epoches}.pt'))
                ...
                while sentence != 'Exit':
                    ...
                    output, hidden = encoder(enData, length)
                    ...
                    while cnItos[index] != '<eos>':
                        ...
                        count += 1
                        # Getting the prediction.
                        prediction, hidden = decoder(prediction, [1], output, hidden)
                        # Getting the index.
